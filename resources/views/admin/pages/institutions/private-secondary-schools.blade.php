        ...
        <div class="">
            <div class="page-title">
                <div class="title_left">
                    <h3>Private Secondary Schools  </h3>
                </div>
                    ...
                    <div class="x_panel">
                        ...
                        <div class="x_content">
                            ...
                            <table id="datatable" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                @foreach($privatesecondary as $item)
                                    <tr class="even pointer">
                                        {{--@foreach($user->institutions as $institution)--}}

                                        <td class=""> {{$item->id}}</td>
                                        <td class=""> {{$item->institution_name}}</td>
                                        <td class=""> {{$item->activity}}</td>
                                        <td class=""> {{$item->upcoming}}</td>
                                        <td class=""> {{$item->outcome}}</td>

                                        @if($item->orders != null)
                                            <td class=""> {{$item->orders}}</td>
                                        @else
                                            <td class="">No orders taken</td>


                                        @endif
                                        <td class=""> {{$item->region->region}}</td>
                                        <td class=""> {{$item->county->county}}</td>
                                        <td class=""> {{$item->contactName}}</td>

                                        @if($item->contactEmail!=null)
                                            <td class=""> {{$item->contactEmail}}</td>
                                        @else
                                            <td class="">N/A</td>
                                        @endif
                                        <td class=""> {{$item->contactDesignation}}</td>
                                        <td class=""> {{$item->contactNumber}}</td>
